...
pub trait AuthorizeCookie : CookieId {
    ...
    fn retrieve_cookie(_: String) -> Option<Self> where Self: Sized;
...
pub trait AuthorizeForm : CookieId {
    ...
    fn new_form(_: &str, _: &str, _: Option<HashMap<String, String>>) -> Self;
    ...
    fn flash_redirect(&self, ok_redir: impl Into<String>, err_redir: impl Into<String>, cookies: &mut Cookies, connection: DbConnection) -> Result<Redirect, Flash<Redirect>> {
        match self.authenticate(connection) {
            Ok(cooky) => {
    ...
    fn redirect(&self, ok_redir: &str, err_redir: &str, cookies: &mut Cookies, connection: DbConnection) -> Result<Redirect, Redirect> {
        match self.authenticate(connection) {
            Ok(cooky) => {
                let cid = Self::cookie_id();
                let contents = cooky.store_cookie();
